#!/usr/bin/env python3
"""Pipeline 10 runner: batched graph/RAG rule extraction packs.

Pipeline 10 keeps Pipeline 9's generalized graph/RAG selection path, then
changes the LLM transport layer:
...
def cached_adapter_needs_table_refresh(manifest: dict[str, Any], source_visual_dir: Path | None) -> bool:
    ...
    if not source_table_path.exists() or source_table_path.stat().st_size == 0:
        return False
    return int(manifest.get("table_region_count") or 0) == 0
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        "pipeline": "official_rule_extraction_pipeline_batched_graph_rag_rule_extraction",
        "city": args.city,
    ...
    if not args.skip_adapter:
        ...
        source_visual_blocks_dir = args.source_visual_blocks_dir or default_source_visual_blocks_dir(args.city)
        cached_manifest = read_json(manifest_path) if manifest_path.exists() else None
        if (
            cached_manifest
            and not args.overwrite
            and not cached_adapter_needs_table_refresh(cached_manifest, source_visual_blocks_dir)
        ):
            ...
            summary["stages"]["rag_visual_block_adapter"] = adapt_rag_packs(
                Namespace(
                    ...
                    target_terms=config.get("target_terms", []),
                    parent_terms=config.get("parent_terms", []),
                    source_visual_blocks_dir=source_visual_blocks_dir,
    ...
    write_json(city_dir / "pipeline10_summary.json", summary)
    return summary
...
def compact_run_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "full_summary_path": str((Path(summary["output_dir"]) / "pipeline10_summary.json").resolve()),
    }
