...
def process_batched_extraction(args: argparse.Namespace) -> dict[str, Any]:
    ...
    def extract_text_batch(batch: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    def extract_table_region(region: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    def run_jobs(items: list[dict[str, Any]], worker) -> list[tuple[int, list[dict[str, Any]], dict[str, Any]]]:
        if not items:
            return []
        ...
        if max_workers == 1:
            ...
            for index, item in enumerate(items):
                rules, log = worker(item)
                serial_results.append((index, rules, log))
                if args.call_delay_seconds > 0 and index < len(items) - 1:
                    time.sleep(args.call_delay_seconds)
            return serial_results

        parallel_results: list[tuple[int, list[dict[str, Any]], dict[str, Any]]] = []
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                rules, log = future.result()
                parallel_results.append((index, rules, log))
        return sorted(parallel_results, key=lambda row: row[0])

    table_results_future = None
    table_stage_executor = None
    if not only_batches and regulatory_tables:
        # Text and table branches are independent, so start table work while text
        # batches are still running. Each branch still preserves deterministic
        # per-branch output ordering after futures complete.
        table_stage_executor = ThreadPoolExecutor(max_workers=1)
        table_results_future = table_stage_executor.submit(run_jobs, regulatory_tables, extract_table_region)

    for _, rules, log in run_jobs(batches, extract_text_batch):
    ...
    if only_batches:
        ...
        table_results = table_results_future.result() if table_results_future is not None else run_jobs(regulatory_tables, extract_table_region)
        if table_stage_executor is not None:
            table_stage_executor.shutdown()
        for _, rules, log in table_results:
    ...
    for index, rule in enumerate(combined_rules, start=1):
        rule["rule_id"] = f"pipeline10_rule_{index:04d}"
    ...
    summary = build_summary(
        ...
        table_model=table_model,
        dry_run=False,
        wall_clock_seconds=time.time() - wall_started,
    )
    write_json(output_dir / "summary.json", summary)
    return summary
...
def build_summary(
    *,
    ...
    return {
        "pipeline": "official_rule_extraction_pipeline_batched_rag_rule_extraction",
        "dry_run": dry_run,
        "model": args.model,
        "text_model": text_model,
        "table_model": table_model,
