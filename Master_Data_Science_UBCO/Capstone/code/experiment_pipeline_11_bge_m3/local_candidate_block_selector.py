...
OFFICIAL_PDF_DIR = PIPELINE_DIR.parent / "official_rule_extraction_pipeline" / "pdfs"
...
def configured_pdf_path(config: dict[str, Any], override: Path | None = None) -> Path:
    if override:
        return override.resolve()
    local_path = PDF_DIR / config["pdf"]
    if local_path.exists():
        return local_path.resolve()
    official_path = OFFICIAL_PDF_DIR / config["pdf"]
    if official_path.exists():
        return official_path.resolve()
    return local_path.resolve()
...
def process_local_selection(args: argparse.Namespace) -> dict[str, Any]:
    ...
    pdf_path = configured_pdf_path(config, args.pdf)
