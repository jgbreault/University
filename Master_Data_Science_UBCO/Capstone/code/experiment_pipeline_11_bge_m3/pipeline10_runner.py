#!/usr/bin/env python3
"""Pipeline 11 runner: hybrid graph/RAG rule extraction packs.

Pipeline 11 keeps Pipeline 10's generalized graph/RAG selection and batched
extraction path, then changes the compression layer:

1. deterministic local candidate selection;
2. automatic discovery/audit of candidate rule sections;
3. hybrid lexical + local bge-m3 embedding compression;
4. graph/RAG pack construction with rule lanes;
5. conversion into legacy visual-block input;
6. optional batched target-scoped rule extraction with preflight filtering,
   checkpoint/cache files, and retry/backoff.
...
RESOURCE_VISUAL_BLOCKS_DIR = ROOT / "resources" / "visual_blocks"
OFFICIAL_RESOURCE_VISUAL_BLOCKS_DIR = ROOT.parent / "official_rule_extraction_pipeline" / "resources" / "visual_blocks"
...
from batched_rag_rule_extractor import process_batched_extraction  # noqa: E402
...
def default_source_visual_blocks_dir(city: str) -> Path | None:
    candidates = [
        RESOURCE_VISUAL_BLOCKS_DIR / city,
        OFFICIAL_RESOURCE_VISUAL_BLOCKS_DIR / city,
        ROOT / "outputs" / city / "03_visual_blocks",
    ]
    for candidate in candidates:
        if (candidate / "table_regions.jsonl").exists():
            return candidate
    return None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--compression-provider", choices=["lexical", "ollama", "hybrid"], default="hybrid")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional visual-blocks dir used to inherit table_regions/table_images.",
    )
    parser.add_argument("--run-extraction", action="store_true", help="Call the batched LLM rule extractor.")
    parser.add_argument("--dry-run-extraction", action="store_true", help="Build extraction batches without API calls.")
    parser.add_argument("--only-batches", nargs="*", default=[], help="Run only these text_batch ids in rule extraction.")
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    rules_dir = city_dir / "06_rule_extraction_batched"

    summary: dict[str, Any] = {
        "pipeline": "experiment_pipeline_11_bge_m3_hybrid_graph_rag_rule_extraction",
        "city": args.city,
        ...
        if manifest_path.exists() and not args.overwrite:
            ...
            summary["stages"]["rag_visual_block_adapter"] = adapt_rag_packs(
                Namespace(
                    ...
                    target_terms=config.get("target_terms", []),
                    parent_terms=config.get("parent_terms", []),
                    source_visual_blocks_dir=args.source_visual_blocks_dir
                    or default_source_visual_blocks_dir(args.city),
    ...
    if args.run_extraction or args.dry_run_extraction:
        summary["stages"]["rule_extraction"] = process_batched_extraction(
            Namespace(
                ...
                overwrite=args.overwrite,
                target_terms=config.get("target_terms", []),
                dry_run=args.dry_run_extraction and not args.run_extraction,
            )
        )
    else:
        summary["stages"]["rule_extraction"] = {
            "status": "skipped",
            "reason": "pass --run-extraction to call the batched LLM extractor",
            "input_visual_blocks_dir": str(visual_dir),
            "planned_output_dir": str(rules_dir),
        }

    write_json(city_dir / "pipeline11_summary.json", summary)
    return summary
...
def compact_run_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_compression": {
            ...
            "scoring_policy": compression.get("scoring_policy"),
            "ollama_model": compression.get("ollama_model"),
            "embedding_cache": compression.get("embedding_cache"),
        ...
        "full_summary_path": str((Path(summary["output_dir"]) / "pipeline11_summary.json").resolve()),
    }
