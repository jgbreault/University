...
def run_case(city: str, rule_dir: Path, strict_visual_dir: Path, output_dir: Path) -> dict[str, Any]:
    rules = read_json(rule_dir / "text_rules_raw.json")
    ...
    summary = {
        ...
        "rule_dir": str(rule_dir.resolve()),
        "strict_visual_dir": str(strict_visual_dir.resolve()),
...
def main() -> None:
    ...
    cases = [
        (
            "burnaby",
            ROOT / "outputs" / "burnaby" / "06_rule_extraction_hq",
            ROOT / "outputs" / "burnaby" / "05_rag_visual_blocks",
        ),
        (
            "vancouver",
            ROOT / "outputs" / "vancouver" / "06_rule_extraction_hq",
            ROOT / "outputs" / "vancouver" / "05_rag_visual_blocks",
        ),
    ]
